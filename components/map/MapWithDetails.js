...
// ====================================================================
const ReportModal = ({ isOpen, onClose, store }) => {
...
// ====================================================================
// کامپوننت سایدبار لیست فروشگاه‌ها (تغییر یافته)
// ====================================================================
const StoreListSidebar = ({ stores, onStoreSelect, selectedStoreIdx, onReportClick }) => {
    return (
            ...
            <div className="flex-grow overflow-y-auto">
                {stores.map((store, index) => (
                    ...
                    >
                        ...
                        <div className="mb-3">
                            <span className="text-green-600 font-semibold">{store.performance}</span>
                        </div>
...
export default function MapWithStoreListPage({ initialCenter = null }) {
  ...
  const staticMarkers = [
    { id: 1, lat: 35.6892, lng: 51.389, name: "سینا یدک", city: "کرج", performance: "عملکرد عالی", desc: "خیابان ملت، خیابان کاوه، کوچه افسانه، پلاک ۷، طبقه دوم، واحد ۶", price: "۱۰۱,۴۹۹,۰۰۰", phone: "09121234567", whatsapp: "989121234567" },
    { id: 2, lat: 35.6895, lng: 51.390, name: "امید یدک", city: "تهران", performance: "عملکرد خوب", desc: "خیابان آزادی، نزدیک فروشگاه اصغر", price: "۹۹,۵۰۰,۰۰۰", phone: "09129876543", whatsapp: "989129876543" },
    { id: 3, lat: 35.6888, lng: 51.388, name: "پارس یدک", city: "تهران", performance: "عملکرد عالی", desc: "خیابان آزادی، روبروی فروشگاه اصغر", price: "۱۰۵,۰۰۰,۰۰۰", phone: "09125551234", whatsapp: "989125551234" },
  ];
  ...
  useEffect(() => {
    ...
    staticMarkerRefs.current = staticMarkers.map(
        ({ lat, lng, name, price }, idx) => {
          ...
          label.innerText = price ? `${name} (${price} ت)` : name;
          label.className = "bg-black/70 text-white text-[10px] px-2 py-0.5 rounded-lg mt-1 whitespace-nowrap";
          wrapper.appendChild(label);

          wrapper.onclick = () => handleStoreSelect({ lat, lng }, idx);
