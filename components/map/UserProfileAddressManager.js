...
const NeshanSearchControl = ({ map, apiKey }) => {
  ...
  return (
    <div
      ...
      className="absolute top-4 left-1/2 -translate-x-1/2 z-20 w-[90%] max-w-sm"
      dir="rtl"
    >
      <div className="relative">
        <input
          ...
          className={`bg-white w-full pl-10 pr-4 py-2.5 border-2 border-gray-300 shadow-lg outline-none transition-all ${suggestions.length > 0 && isFocused ? 'rounded-t-lg' : 'rounded-lg'}`}
        />
        ...
        {isOpen && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[4000] p-4" >
            <motion.div
              ...
              className="relative bg-white rounded-lg shadow-xl w-full max-w-3xl h-[70vh] flex flex-col overflow-hidden border-gray-200 p-5 px-6"
            >
              <button
                onClick={onClose}
              ...
              <h3 className="p-2 text-xl font-bold text-gray-800 text-right">
               انتخاب موقعیت مکانی
              </h3>
              <p className="px-4 pb-4 text-xs text-gray-500">سفارش‌های شما به موقعیتی که انتخاب می‌کنید ارسال می‌شود. لطفاً موقعیت مکانی را دقیق انتخاب کنید تا از تأخیر در تحویل جلوگیری شود.</p>
              <div className="relative flex-grow bg-gray-200">
                ...
                <div
                  className="absolute bottom-4 right-4 z-20 bg-white/80 backdrop-blur-sm p-3 rounded-lg shadow-md max-w-xs "
                  dir="rtl"
                >
                    <div className="flex items-center  bg-amber-500 w-fit rounded-lg p-2">
                  <p className="text-sm font-semibold  p-2 text-white rounded-lg">یدکچی</p>
                  <img src={YadakchiLogo.src} alt="Yadakchi Logo" className="w-6 h-8" />
                  </div>
                  <p className="text-xs text-gray-600 mt-3">
                    پین را روی مکان دقیق خود قرار دهید و سپس دکمه "تایید این مکان" را بزنید.
                  </p>
                </div>
              </div>
              <div className="p-4 bg-gray-50 border-t flex justify-center gap-4">
                <button
                  ...
                  className="px-8 py-3 bg-blue-600 text-white font-bold rounded-lg shadow-lg hover:bg-blue-700 transition-colors cursor-pointer w-1/2"
                >
                  تایید این مکان
                </button>
                <button
                  onClick={onClose}
                  className="px-8 py-3 bg-gray-500 text-white font-bold rounded-lg hover:bg-gray-600 transition-colors cursor-pointer w-1/2"
                >
                  انصراف
                </button>
              </div>
            ...
            {isFetching ? (
              ...
              <>
                <div ref={imageContainerRef} className="relative w-full h-48 rounded-t-lg bg-gray-200 flex items-center justify-center overflow-hidden">
                  {locationInfo?.mapImage ? (
                    ...
                    <div className="text-center text-gray-500 p-4">
                      ...
                      <p className="text-sm">برای تغییر مکان پین، آدرس را حذف و دوباره ثبت کنید.</p>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 bg-gray-50 border-t flex justify-between items-center">
                  {!isEditMode ? (
                    <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 rounded-md hover:bg-gray-200 transition-colors cursor-pointer">
                      <FaArrowRight />
                      <span>بازگشت به نقشه</span>
                    </button>
                  ) : (<div />)}
                  <div className="flex gap-3">
                    <button onClick={onClose} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors cursor-pointer">
                      لغو
                    </button>
                    <button onClick={handleSave} disabled={isSaving} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-md hover:bg-blue-700 transition-colors cursor-pointer flex items-center justify-center disabled:opacity-50">
    ...
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[6000] p-4">
        ...
        >
          ...
          <p className="text-sm text-gray-500 mt-2">آیا از حذف این آدرس مطمئن هستید؟ این عمل غیرقابل بازگشت است.</p>
          <div className="mt-6 flex justify-center gap-4">
            <button onClick={onClose} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 cursor-pointer">
              انصراف
            </button>
            <button onClick={onConfirm} className="px-6 py-2 bg-red-600 text-white font-bold rounded-md hover:bg-red-700 cursor-pointer">
              بله، حذف کن
            </button>
          </div>
        </motion.div>
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="relative"
            >
              <img src={imageUrl} alt="نمای بزرگ شده نقشه" className="max-w-[90vw] max-h-[90vh] rounded-lg shadow-2xl"/>
              <button onClick={onClose} className="absolute -top-3 -right-3 z-10 p-1.5 bg-white text-black rounded-full shadow-lg hover:scale-110 transition-transform" aria-label="بستن تصویر">
                <IoMdClose size={24} />
              </button>
            </motion.div>
          </motion.div>
...
export default function UserAddressManager() {
  ...
  return (
    <div className="p-8 font-sans bg-gray-50 min-h-screen border-2 border-gray-300 rounded-lg" dir="rtl">
      <motion.div
      ...
      >
        <motion.div
          className="bg-white p-6 rounded-lg shadow-md border-2 border-gray-200 flex justify-between items-center"
          variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }}
        >
          <h2 className="text-2xl font-bold mb-4 border-b-2 border-gray-200 pb-4 w-1/5">مدیریت آدرس‌ها</h2>
          <div className="pt-6">
            <motion.button
        ...
        <motion.div className="mt-8" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }}>
          <motion.h3
              ...
              <AnimatePresence>
                {userAddresses.map((addr) => (
                  <motion.div
                    key={addr.id}
                    layout
                    initial="hidden" animate="visible" exit={{ opacity: 0, x: -50, height: 0, padding: 0, margin: 0 }}
                    variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }}
                    className="p-4 rounded-lg bg-white shadow-md flex items-start justify-between gap-4"
                  >
                    {addr.mapImage && (
                        <button onClick={() => setImageToView(addr.mapImage)} className="flex-shrink-0">
                            <img src={addr.mapImage} alt="نمای نقشه آدرس" className="w-24 h-24 object-cover rounded-md shadow-sm cursor-pointer hover:shadow-lg hover:scale-105 transition-all duration-200"/>
                        </button>
                    )}
                    <div className="flex-grow">
                      <p className="font-bold flex items-center gap-2 text-gray-800">
                        <FaMapMarkerAlt className="text-blue-500" /> {addr.fullAddress}
                      </p>
                      <p className="text-sm text-gray-600 mt-2">
                        {addr.province}، {addr.city} - پلاک {addr.plak}{addr.vahed && `، واحد ${addr.vahed}`} - کدپستی: {addr.postalCode}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => handleEditAddress(addr)} className="p-2 text-gray-500 hover:text-blue-600 rounded-full hover:bg-gray-100 transition-colors cursor-pointer" title="ویرایش">
                          <FaPencilAlt size={16} />
                        </button>
                        <button onClick={() => startDeleteProcess(addr.id)} className="p-2 text-gray-500 hover:text-red-600 rounded-full hover:bg-gray-100 transition-colors cursor-pointer" title="حذف">
                          <FaTrash size={16} />
                        </button>
                    </div>
                  </motion.div>
      ...
      <MapPickerModal isOpen={isMapModalOpen} onClose={handleCloseAllModals} onConfirm={handleLocationConfirm} initialCenter={addressToEdit?.coords}/>
      <AddressFormModal isOpen={isFormModalOpen} onClose={handleCloseAllModals} onSave={handleSaveAddress} locationInfo={selectedLocationInfo} onBack={handleBackToMap}/>
      <ConfirmDeleteModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete}/>
      <ImageViewerModal isOpen={!!imageToView} onClose={() => setImageToView(null)} imageUrl={imageToView}/>
    </div>
  );
}